...
public class ResultView {
    public void printLottos(Lottos lottos) {
        printPurchasingAmount(lottos);
        lottos.getLottos().stream().map(Lotto::toString).forEach(System.out::println);

    }

    private void printPurchasingAmount(Lottos lottos) {
        System.out.println("수동으로 " + lottos.getManualCount() + "장, 자동으로 " + lottos.getAutoSize() + "개를 구매했습니다.");
